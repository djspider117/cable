...
vec3 pal(float t){return .5+.5*cos(6.28*(1.*t+vec3(.0,.1,.1)));}
 float stepNoise(float x, float n) { // From Kamoshika shader
   ...
   float res = mix(floor(fract(sin(i) * 43758.5453123) * n), floor(fract(sin(i + 1.) * 43758.5453123) * n), u);
   res /= (n - 1.) * 0.5;
   return res - 1.;
 }
 vec3 path(vec3 p){
...
vec4 main( vec2 fragCoord )
{
 ...
 vec3 col = vec3(0.);
  ...
  float i=0.,e=0.,g=0.;
  vec3 rd = mat3(x,cross(z,x),z)*erot(normalize(vec3(uv,1.)),vec3(0.,0.,1.),stepNoise(iTime+fract(sin(uv.x*uv.y*iTime) * 43758.5453123)*.05,6.));
